...
config_filename = '/home/rpz21/python/toggle_HS_client_config.txt'
config = {}
try:
	...
	error_code = f'config file: {config_filename} not found, exiting within 60 seconds'
	print(error_code)
...
print(start_time)
while True:
	if button.is_pressed and oled_switched == True:
		if check_if_hs_active(get_ip_address(interface)):
			current_mode = 2
		else:
			current_mode = 1
		...
		oled_switched = False
		continue
	if button.is_pressed and oled_switched == False:
	...
	if not switched_modes and button_pressed:
		...
		draw.text((0,12), message, font=font, fill=255,)
		oled.image(image)
		oled.show()
